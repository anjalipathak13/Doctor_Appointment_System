...
//hi
	if($_POST['action'] == 'patient_register')
	{
		...
		else
		{
			...
			$mail->Port = '80';
			$mail->SMTPAuth = true;
			$mail->Username = 'akshatvideos89@gmail.com';
			$mail->Password = 'F4A106CFB951FCEDBD36DB8EA2E8F7E640EE';
			$mail->SMTPSecure = '';
			$mail->From = 'akshatvideos89@gmail.com';
			$mail->FromName = 'DAS ADMIN';
			...
			if($mail->Send())
			{
				$success = '<div class="alert alert-success">Please Login</div>';
			}
			// else
			// {
	...
	if($_POST['action'] == 'patient_login')
	{
		...
		if($object->row_count() > 0)
		{
			...
			{
				if($row["email_verify"] == 'Yes' || $row["email_verify"] == 'No' )
				{
